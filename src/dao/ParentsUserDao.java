		...
		try{
			...
			if (old == null) {
				...
				statement = connection
						.prepareStatement("update parentsuser set parents_id=?, parents_name=?,parents_pass=? ,parents_address=?, parents_tel=?, parents_mail1=?, parents_mail2=?, parents_mail3=?, facility_id=? where parents_id=? and facility_id=?");
				//プリペアードステートメントに値をバインド
				statement.setString(1, pu.getParents_id());
				statement.setString(2, pu.getParents_name());
				statement.setString(3, pu.getParents_pass());
	...
	public List<String> getParentsEmails(String child_id) throws Exception {
	    ...
	    try (Connection connection = getConnection();
	        ...
	        try (ResultSet rs = statement.executeQuery()) {
	            if (rs.next()) {
	                // 各メールをリストに追加（nullを除外）
	                String mail1 = rs.getString("parents_mail1");
	                String mail2 = rs.getString("parents_mail2");
	                String mail3 = rs.getString("parents_mail3");

	                if (mail1 != null) emails.add(mail1);
	                if (mail2 != null) emails.add(mail2);
	                if (mail3 != null) emails.add(mail3);
	            }
	        }
	    }
	    return emails;
